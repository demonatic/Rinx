...
inline std::string http_status_table[512];

static void init_http_status_table(){
#define TABLE_ENTRY_INSERT(num,name,string) http_status_table[num]=#num " " #string;
...
inline std::string to_http_status_code_str(HttpStatusCode status_code){
    ...
    return detail::http_status_table[Util::to_index(status_code)];
}

static constexpr size_t MethodCount=Util::to_index(HttpMethod::__HttpMethodCount);
inline const std::array<std::string,MethodCount> str_methods{"GET","POST","HEAD","PUT","PATCH","DELETE","OPTIONS","ANY","UNDEFINED"};
...
inline const std::array<std::string,VersionCount> str_versions{"HTTP/1.0","HTTP/1.1","HTTP/2","UNKNOWN"};
