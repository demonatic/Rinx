...
public:
    ...
    static void default_static_file_handler(HttpRequest &req,HttpResponse &resp,const std::string &web_root_dir,const std::string &default_page);
...
private:
    ...
    void set_timeout(uint64_t sec){
        ...
        _read_timer.start_timer(sec*1000,[this](){
            ...
            conn->get_eventloop()->queue_work([=](){conn->close();});
        });
